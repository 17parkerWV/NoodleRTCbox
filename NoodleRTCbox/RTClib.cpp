...
*/
/**************************************************************************/
void RTC_DS3231::adjust(uint8_t yOff, uint8_t m, uint8_t d, uint8_t hh, uint8_t mm, uint8_t ss, uint8_t dotw ) {
  Wire.beginTransmission(DS3231_ADDRESS);
  Wire._I2C_WRITE((byte)DS3231_TIME); // start at location 0
  Wire._I2C_WRITE(bin2bcd(ss));
  Wire._I2C_WRITE(bin2bcd(mm));
  Wire._I2C_WRITE(bin2bcd(hh));
  // The RTC must know the day of the week for the weekly alarms to work.
  Wire._I2C_WRITE(bin2bcd(dowToDS3231(dotw)));
  Wire._I2C_WRITE(bin2bcd(d));
  Wire._I2C_WRITE(bin2bcd(m));
  Wire._I2C_WRITE(bin2bcd(yOff));
  Wire.endTransmission();
